...
class ListTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        else
        {
            vc.modifyCallback =
            {
                self.personList.insert(vc.person!, at: 0)
                self.tableView.reloadData()
            }
        }
    }
    ...
    func loadData(param: @escaping (_ result: [Person]) -> ())
    {
        DispatchQueue.global().async {
            ...
            for i in 0..<30 {
                let person = Person()
                person.name = "name_\(i)"
                person.phone = "186" + String(format: "%7d", arc4random_uniform(10000000))
